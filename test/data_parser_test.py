...
def create_segment(name, fields):
...
class TestDataParser(unittest.TestCase):
    ...
    def test_fields_iter_should_return_generator_when_called(self):
        # Arrange, Act, Assert
        assert isinstance(dp._fields_iter(''), types.GeneratorType)

    def test_fields_iter_should_raise_type_error_when_raw_text_is_none(self):
        # Arrange, Act, Assert
        with self.assertRaises(TypeError):
            dp._fields_iter(None)
    
    def test_fields_iter_should_return_zero_segments_when_raw_text_is_empty(self):
        # Arrange, Act, Assert
        assert len(list(dp._fields_iter(''))) == 0
    
    def test_fields_iter_should_return_zero_segments_when_raw_text_is_single_pipe(self):
        # Arrange, Act, Assert
        assert len(list(dp._fields_iter('|'))) == 0
    
    def test_fields_iter_should_return_multiple_fields_when_raw_text_contains_single_pipe(self):
        # Arrange, Act, Assert
        assert len(list(dp._fields_iter('FOOspam|BAReggs'))) == 2
    
    def test_fields_iter_should_return_field_when_raw_text_contains_field(self):
        # Arrange, Act, Assert
        assert type(list(dp._fields_iter('FOOspam'))[0]) == dp.Field

    def test_fields_iter_should_return_field_with_name_when_raw_text_does_not_contain_single_pipe(self):
        # Arrange, Act, Assert
        assert list(dp._fields_iter('FOO'))[0].name == 'FOO'

    def test_fields_iter_should_return_field_with_value_empty_string_when_raw_text_does_not_contain_single_pipe(self):
        # Arrange, Act, Assert
        assert list(dp._fields_iter('FOO'))[0].value == ''

    def test_fields_iter_should_return_field_with_value_field_when_raw_text_does_not_contain_single_pipe(self):
        # Arrange, Act, Assert
        assert list(dp._fields_iter('FOOspam'))[0].value == 'spam'
    ...
    def test_segment_should_set_fields_to_generator_when_initialized(self):
        # Arrange, Act, Assert
        assert isinstance(dp.Segment('MTY|FOOspam').fields, types.GeneratorType)
    ...
    def test_segment_compose_should_return_self_when_called(self):
        # Arrange
        with patch("src.data_parser._fields_iter", return_value=[]):
    ...
    def test_segment_compose_should_call_fields_iter_with_field_raw_text_when_called(self):
        # Arrange, Act
        with patch("src.data_parser._fields_iter", return_value=[]) as iter_fields_mock:
            dp.Segment('MTY|FOOspam').compose()

            # Assert
            iter_fields_mock.assert_called_once_with('FOOspam')

    def test_segment_compose_should_update_fields_when_called(self):
        # Arrange, Act
        fields = []
        with patch("src.data_parser._fields_iter", return_value=fields):
            segment = dp.Segment('MTY|FOOspam').compose()
    ...
    def test_data_parser_should_set_segments_to_generator_when_initialized(self):
        # Arrange, Act, Assert
        assert isinstance(dp.DataParser().segments, types.GeneratorType)
    
    def test_data_parser_should_set_segments_as_empty_when_initialized(self):
        # Arrange, Act, Assert
        assert len(list(dp.DataParser().segments)) == 0

    def test_data_parser_segments_iter_should_return_segments_generator_when_called(self):
        # Arrange
        segments = (x for x in [
            create_segment('MTY', [dp.Field('FOO', 'spam'), dp.Field('BAR', 'eggs')]),
            create_segment('GRL', [dp.Field('BAZ', 'sir_robin')])])
        data_parser = dp.DataParser()
        data_parser.segments = segments

        # Act, Assert
        assert len(list(data_parser.fields_iter())) == 3

    def test_data_parser_fields_iter_should_return_fields_when_called(self):
        # Arrange
        empty_generator = (x for x in [])
        data_parser = dp.DataParser()
        data_parser.segments = empty_generator
    ...
    def test_data_parser_digest_should_digest_raw_text_when_called(self):
        # Arrange
        segments = [create_segment('MTY', [dp.Field('FOO', 'spam')])]
        with patch("src.data_parser._segments_iter", return_value=segments):
            ...
            # Assert
            assert data_parser.segments == segments
    
    def test_data_parser_search_segments_should_find_segments_when_searched_by_segment_name(self):
        # Arrange
        segment = create_segment('GRL', [dp.Field('BAZ', 'sir_robin')])
        segments = (x for x in [
            create_segment('MTY', [dp.Field('FOO', 'spam'), dp.Field('BAR', 'eggs')]),
            segment])
        data_parser = dp.DataParser()
        ...
        # Act, Assert
        assert list(data_parser.search_segments('GRL'))[0] == segment

    def test_data_parser_search_segments_should_find_segments_when_searched_by_segment_name_and_field_name(self):
        # Arrange
        segments = (x for x in [
            create_segment('MTY', [dp.Field('FOO', 'spam'), dp.Field('BAR', 'eggs')]),
            create_segment('GRL', [dp.Field('BAZ', 'sir_robin')]),
            create_segment('GRL', [dp.Field('BAZ', 'king_arthur')]),
            create_segment('GRL', [dp.Field('QUXX', 'sir_lancelot')])])
        data_parser = dp.DataParser()
        data_parser.segments = segments

        # Act, Assert
        assert len(list(data_parser.search_segments('GRL', ['BAZ']))) == 2
